...
static QList<double > doubless;
static int str_kol;
static int stolb_kol;

static double ** matrix;//данные с диагнозом
static double *list1;
static double *list2;
                      ...
                      while (!intest.atEnd())
                        {
                             ...
                             for (QString item : line.split(","))
                                {
                                    ...
                                    doubless.append(item.toDouble());
                                    qDebug()<<doubless;

                                    //myMatr.append()

                                    // standardItemsList.append(new QStandardItem(doubles[0]));

                                }


                             QVector <double> myVect;


                             //QVector2D <double> mine;
                             ...
                             for(int i = 0; i < doubless.length(); i++)
                               {
                                   myVect.append(doubless[i]);

                                   qDebug() << "Value " << i << ": " << myVect.value(i)<<" "<<doubless[i];
                               }
                             ...
                             str_kol=b;
                             stolb_kol=a;
                             matrix = new double*[str_kol];
                             for (int i = 0; i < str_kol; i++)
                               {
                                   matrix[i] = new double[stolb_kol];
                               }
                             ...
                             for (int i = 0; i < str_kol; i++)
                               {
                                   for (int j=0;j<stolb_kol;j++)
                                       {
                                           matrix[i][j] = doubless[i*stolb_kol+j];
                                           qDebug()<<matrix[i][j];
                             ...
                             //qDeleteAll(doubless.begin(),doubless.end());
                             //doubless.clear();
...
void MainWindow::on_pushButton_2_clicked()
{
    ...
    list1 = new double [str_kol];
    list2 = new double [str_kol];
...
{     int k;k=0;
     ...
     //определяем оптимальную точку разделения (сумма PH и ND минимальна) первый способ
     int St;
     int Sum;
     int sumj;sumj=0; St=0;Sum=PH[0]+ND[0];
     double opt_COP;St=0;
     for(int i=1;i<len;i++)
        {
         St=PH[i]+ND[i];
         qDebug()<<"PH + ND "<<St<<", i= "<<i<<",cop = "<<massCOP[i];
         if(St<Sum)
            {
             Sum=PH[i]+ND[i]; sumj = i;
             qDebug()<<"coollll PH+ND= "<<Sum<<" ,j = "<<sumj;
            }

        }
     opt_COP=massCOP[sumj];
     ...
     for(int i=0;i<len;i++)
        {
         Se[i]=double(PD[i])/(double(PD[i])+double(ND[i]));
         Spec[i]=double(NH[i])/(double(NH[i])+double(PH[i]));
        }
     ...
     //второй способ max se+spec
     double St_2;
     double Sum_2;
     int sumj_2;sumj_2=0;Sum_2=Se[0]+Spec[0];
     double opt_COP_2;St_2=0;
     for(int i=0;i<len;i++)
        {
         St_2=Se[i]+Spec[i];
         qDebug()<<"Se + Spec "<<St_2<<", i= "<<i<<",cop = "<<massCOP[i];
         if(St_2>Sum_2)
            {
             Sum_2=Se[i]+Spec[i]; sumj_2 = i;
             qDebug()<<"cool Se+Spec= "<<Sum<<" ,j = "<<sumj_2;
            }

        }
     opt_COP_2=massCOP[sumj_2];
     qDebug()<<"optCOP_2 "<<opt_COP_2;
   ...
   qDebug()<<"out to screen";
   qDebug()<<"opt_cop"<<opt_COP;
   qDebug()<<"spec"<<Spec[sumj];


   std::string str = std::to_string(Spec[sumj_2]);

   string lllloo; lllloo = str;
   ...
   //cop_opt.append("Оптимальная граница разделения классов 2 : ");
   cop_opt.append(QString::number(double(opt_COP_2)));

   j_opt = "номер точки : ";
   //j_opt.append(QString::number(sumj));
   //j_opt.append("номер точки 2: ");
   j_opt.append(QString::number(sumj_2));

   sum_err = "Сумма ошибок для  границы: ";
   //sum_err.append(QString::number(Sum));
   //sum_err.append("Сумма ошибок для 2 границы: ");
   sum_err.append(QString::number(ND[sumj_2]+ PH[sumj_2]));

   Spec_str = "Специфичность: ";
   //Spec_str.append(QString::number(double(Spec[sumj])));
   //Spec_str.append("Специфичность 2 : ");
   Spec_str.append(QString::number(double(Spec[sumj_2])));


   senc_str = "Чувствительность ";
   //senc_str.append(QString::number(double(Se[sumj])));
   //senc_str.append("Чувствительность 2");
   senc_str.append(QString::number(double(Se[sumj_2])));
...
double MainWindow::minfunc(double * list)
{   double mind;
    mind=list[0];
...
double MainWindow::maxfunc(double * list)
{   double maxd;
    maxd=list[0];
    for(int i=1;i<str_kol;i++)
        {if(list[i]>maxd)
...
void MainWindow::on_pushButton_4_clicked()
{
   ...
   //Вычисляем наши данные
       QVector<double> X(len+1), Y(len+1);
       for (int i =0;i<len;i++)
        {
         X[i]=no_sp[i];
         Y[i]=Se[i];
